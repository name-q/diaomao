...
class AiService extends Service {
  ...
  async reviewCode(diffContent, commitMessage = '') {
    ...
    try {
      // 截断过长的内容
      const truncatedDiff = this.truncateContent(diffContent, config.review.maxTokens);
...
请以${config.review.style === 'professional' ? '专业严谨' : '友好建议'}的语调提供反馈，并在最后给出总分（1-10分）。`;
...
请提供详细的审查意见和改进建议。`;
  ...
  async callOpenAI(messages) {
    ...
    try {
      const response = await axios.post(`${config.ai.openai.baseURL}/chat/completions`, {
      ...
      ctx.logger.error('OpenAI API调用失败:', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async callDeepSeek(messages) {
    ...
    try {
      ...
      ctx.logger.error('DeepSeek API调用失败:', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  async callQwen(messages) {
    ...
    try {
      const response = await axios.post(`${config.ai.qwen.baseURL}/chat/completions`, {
      ...
      ctx.logger.error('Qwen API调用失败:', error.response?.data || error.message);
      throw error;
    }
  }
  ...
  truncateContent(content, maxTokens) {
    ...
    try {
      ...
      ctx.logger.warn(`内容过长，已截断: ${tokens.length} -> ${maxTokens} tokens`);
      return truncatedContent;
    } catch (error) {
      ctx.logger.error('Token计算失败，使用字符截断:', error);
      // 降级到字符截断
      return content.length > maxTokens * 4 ? content.substring(0, maxTokens * 4) : content;
    }
  }
